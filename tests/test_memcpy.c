...
int	main(void)
{
	...
	printf("[%d] memcpy(dst, src, 14) (NUL 含む)  expect=libc と一致  r=%p  %s%s%s\n",
		2, r, color, label, reset);

	memset(dst1, 0xAA, 16);
	memset(dst2, 0xAA, 16);
	r = ft_memcpy(dst1, src, 0);
	pass = (memcmp(dst1, dst2, 16) == 0);
	if (pass)
	{
		color = green;
	...
	printf("[%d] memcpy(dst, src, 0)  expect=変化なし  r=%p  %s%s%s\n",
	...
	printf("[%d] 戻り値  expect=dst 自身  ft=%p expect=%p  %s%s%s\n",
		4, r, (void *)dst1, color, label, reset);
